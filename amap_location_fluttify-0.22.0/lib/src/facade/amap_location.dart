...
class AmapLocation {
  ...
  /// 初始化
  Future<void> init({required String iosKey, String? ohosKey}) {
    // 鸿蒙平台暂不在此初始化，原生端会在需要时初始化
    if (Platform.isOhos) {
      debugPrint('鸿蒙平台：跳过 init() 中的 SDK 初始化');
      return Future.value();
    }
  ...
  Future<void> updatePrivacyShow(bool value) async {
    if (Platform.isOhos) {
      // 鸿蒙平台通过 MethodChannel 调用原生方法
      debugPrint('调用：updatePrivacyShow:$value');
      await kAmapLocationFluttifyChannel.invokeMethod('updatePrivacyShow', {'value': value});
      return;
    }
  ...
  Future<void> updatePrivacyAgree(bool value) async {
    if (Platform.isOhos) {
      // 鸿蒙平台通过 MethodChannel 调用原生方法
      debugPrint('调用：updatePrivacyAgree:$value');
      await kAmapLocationFluttifyChannel.invokeMethod('updatePrivacyAgree', {'value': value});
      return;
    }
  ...
  Future<Location> fetchLocation({
    ...
    if (Platform.isOhos) {
      // 鸿蒙平台通过 MethodChannel 调用原生方法
      debugPrint('调用：fetchLocation');
      final result = await kAmapLocationFluttifyChannel.invokeMapMethod<String, dynamic>('fetchLocation', {
        'mode': mode.index,
      ...
      return Location(
        ...
        latLng: LatLng(
          (result?['latitude'] as num?)?.toDouble() ?? 0,
          (result?['longitude'] as num?)?.toDouble() ?? 0,
        ),
        altitude: (result?['altitude'] as num?)?.toDouble(),
        bearing: (result?['bearing'] as num?)?.toDouble(),
  ...
  Stream<Location> listenLocation({
    ...
    if (Platform.isOhos) {
      // 鸿蒙平台通过 MethodChannel 调用原生方法
      debugPrint('调用：listenLocation');
      _locationController ??= StreamController<Location>();

      // 监听定位结果
      kAmapLocationFluttifyChannel.setMethodCallHandler((call) async {
      ...
      // 开始连续定位
      await kAmapLocationFluttifyChannel.invokeMethod('listenLocation', {
        'mode': mode.index,
        'needAddress': needAddress,
        'timeout': timeout?.inMilliseconds,
        'interval': interval,
  ...
  Future<void> stopLocation() {
    if (Platform.isOhos) {
      // 鸿蒙平台通过 MethodChannel 调用原生方法
      debugPrint('调用：stopLocation');
      return kAmapLocationFluttifyChannel.invokeMethod('stopLocation').then((_) async {
  ...
  Future<void> enableBackgroundLocation(
    ...
    if (Platform.isOhos) {
      // 鸿蒙平台通过 MethodChannel 调用原生方法
      debugPrint('调用：enableBackgroundLocation');
      return kAmapLocationFluttifyChannel.invokeMethod('enableBackgroundLocation', {
        'id': id,
        'bgNotification': {
  ...
  Future<void> disableBackgroundLocation(bool var1) {
    if (Platform.isOhos) {
      // 鸿蒙平台通过 MethodChannel 调用原生方法
      debugPrint('调用：disableBackgroundLocation');
      return kAmapLocationFluttifyChannel.invokeMethod('disableBackgroundLocation', {'var1': var1});
    }
  ...
  Stream<GeoFenceEvent> addPolygonGeoFence({
    ...
    if (Platform.isOhos) {
      // 鸿蒙平台通过 MethodChannel 调用原生方法
      debugPrint('调用：addPolygonGeoFence');
      _geoFenceEventController ??= StreamController<GeoFenceEvent>.broadcast();

      // 创建多边形电子围栏
      await kAmapLocationFluttifyChannel.invokeMethod('addPolygonGeoFence', {
        'pointList': pointList.map((point) => {
          'latitude': point.latitude,
          'longitude': point.longitude,
        }).toList(),
        'customId': customId,
        'activeActions': activeActions.map((action) => action.index).toList(),
      });
